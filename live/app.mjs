...
let liveCanvas=null;
function liveFace(landmarks){ // same crop as the selfie, refreshed every detection so the face on the body is you right now
 ...
 const crop=liveCanvas||(liveCanvas=document.createElement('canvas'));const w=400,h=Math.round(400*(maxY-minY)/(maxX-minX));if(crop.width!==w||crop.height!==h){crop.width=w;crop.height=h}
 const c=crop.getContext('2d');c.setTransform(1,0,0,1,0,0);c.clearRect(0,0,w,h);const pad=12,sx=(w-pad*2)/(maxX-minX),sy=(h-pad*2)/(maxY-minY);
 c.translate(w,0);c.scale(-1,1);c.beginPath();points.forEach((p,i)=>{const x=pad+(p.x-minX)*sx,y=pad+(p.y-minY)*sy;i?c.lineTo(x,y):c.moveTo(x,y)});c.closePath();c.strokeStyle='#fffdf2';c.lineWidth=16;c.lineJoin='round';c.stroke();c.save();c.clip();c.drawImage(video,minX,minY,maxX-minX,maxY-minY,pad,pad,w-pad*2,h-pad*2);c.restore();face=crop;
}
...
function updateFace(now){
 ...
 if(mode!=='setup'&&face)liveFace(landmarks);
 if(mode==='setup'){
